...
<head>
    ...
    <link rel="stylesheet" href="/service/resources/base/css/main.css" />
      ...
      <div class="bgpatti">
        ...
        <c:choose>
           <c:when test="${empty blogPage.pageItems}">
              No Blogs found for what you are looking
            </c:when>
          <c:otherwise>
            ...
            <c:forEach var="blog" items="${blogPage.pageItems}" varStatus="i">
              <li class="bgWhite" data-equalizer-watch>
                <div class="blogDetails">
                  ...
                  <a class="link" href="${context_root}/service/detail?id=${blog.id}">go to article <span class="icon-arrow-right"></span></a>
                </div>
              </li>
          </c:forEach>
           </ul>
          ...
          <ul class="pagination">
                               <c:forEach begin="1" end="${blogPage.pagesAvailable}" varStatus="loop"><!-- ${schoolPage.pagesAvailable} -->
                                <c:choose>
                                  ...
                                  <c:otherwise>
                                   <li><a  class="pagelink" href="${context_root}/service/homepg?pageno=<c:out value="${loop.count}"/>&pagesize=6"><c:out value="${loop.count}"/></a></li>
                                  </c:otherwise>
                                </c:choose>
                      </c:forEach>        
                </ul>
